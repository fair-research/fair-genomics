...
The results of each invocation of this TOPMed RNASeq pipeline results into a BDBag that can be used to compare the validate the reproducibility of the workflow. We are providing a reference [BDBag for outputs](#outputs-provenance-and-performance) that can used to compre the content of the BDBag generated by the users. The outputs BDBag contains a "manifest-md5.txt" file that provides checksums of the ouputs files generated during the analysis. The TOPMed RNAseq pipline also runs RNASeQC that provide the QC results in the form of tables and graphs. These can be used to compare the reprocibility of the ouputs. And example screenshot of the QC metrics is shown below:
